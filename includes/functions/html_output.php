 ...
 */
  function zen_image_submit($image, $alt = '', $parameters = '', $sec_class = '') {
    ...
    {
        $image_submit = '<input type="submit" class="btn btn-success add-to-cart" value="'.$alt.'" />';
    }
    return $image_submit;
  }
 ...
 */
  function zen_draw_form($name, $action, $method = 'post', $parameters = '') {
    $form = '<form role="form" name="' . zen_output_string($name) . '" action="' . zen_output_string($action) . '" method="' . zen_output_string($method) . '"';
