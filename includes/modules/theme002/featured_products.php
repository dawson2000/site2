...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
	...
	$products_name = '<a class="product-name name" href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . substr(strip_tags($featured_products->fields['products_name']), 0, 60) . '</a>';
