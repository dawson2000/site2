...
if ($num_categories > 0) {
  ...
  while (!$categories->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents col-md-3 col-sm-6 col-xs-12"',
                                           'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '"><div data-match-height="catRow" class="subcategory-image ">' . zen_image(DIR_WS_IMAGES . $categories->fields['categories_image'], $categories->fields['categories_name'], SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT) . '</div><span>' . $categories->fields['categories_name'] . '</span></a>');
