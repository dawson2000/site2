...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
	...
	$products_name = '<a class="product-name name" href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . substr(strip_tags($new_products->fields['products_name']), 0, 20) . '</a>';
    
	$products_desc = substr(strip_tags($new_products->fields['products_description']), 0, 60) . '...';
	...
	$products_butt = '<a class="btn products-button" href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_GOTO_PROD_DETAILS, BUTTON_GOTO_PROD_DETAILS_ALT) . '</a>';
  	$products_butt2 = '<a class="btn products-button add-to-cart" href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $new_products->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_ADD_TO_CART, BUTTON_IN_CART_ALT) . '</a>';
    ...
    'text' => 
			...
			'<div class="product-col" data-match-height="new_prod">
				<h5>' . $products_name . '</h5>
				<div class="img">
					' . $products_img . '
					<div class="price">
